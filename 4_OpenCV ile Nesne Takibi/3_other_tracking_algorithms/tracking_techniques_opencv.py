"""
 Takip Algoritmaları:
     ...
     MOSSE Takip Algoritması
     CSRT Takip Algoritması
     KCF (Kernelized Correaliton Filters) Takip Algoritması
...
BOOSTING T.A:
    ...
    Kullanıcı(veya başka bir nesne algılama algoritması tarafından)
    sağlanan ilk sınırlayıcı kutu, nesne için olumlu bir örnek olarak alınır ve
    sınırlayıcı kutunun dışındaki birçok görüntü yaması arka plan olarak kabul edilir.
    
    Yeni bir çerçeve verildiğinde sınıflandırıcı önceki konumun çevresindeki her piksel üzerinde
    çalıştırılır ve sınıflandırıcının puanı kaydedilir
    
    Nesnein yeni konumu puanın maksimum olduğu yerdir.
...
tracker_name = "mil" #boosting bunlardan en zayıf olanı
